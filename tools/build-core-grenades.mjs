 ...
 * ⚠ The Mini-grenade row is NOT shipped: every figure on it is "by grenade" — it is a modifier
 *   applied to the grenade it is loaded as, not an item with numbers of its own.
...
const MINI = '<p>Also comes as a mini-grenade for a grenade launcher (SR3 p.283): same effects, weight .1, and it does not arm until it has travelled five metres.</p>';
...
export const CoreGrenades = { ROWS, buildDoc, fileNameFor };

if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const want = new Map(CoreGrenades.ROWS.map(CoreGrenades.buildDoc).map(d => [CoreGrenades.fileNameFor(d), JSON.stringify(d, null, 2) + '\n']));
  let dirty = 0;
