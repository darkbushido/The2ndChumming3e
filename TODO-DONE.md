...
Not implemented; it belongs with the action economy in [#48](TODO.md#48), which is where the system
...
4. Add tests — this is pure arithmetic with no Foundry dependency, so it belongs in `tests/`
   alongside the damage-code parsers ([#7](TODO.md#7-expand-test-coverage-for-combat-initiative-and-pools)).
...
the deferred rows ([#18](TODO.md#18), visibility, blind fire, multiple targets, walking) drop into place
...
## 36. ✅ Detect which vision an attacker actually has — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
### Blocked on the same gap as [#18](TODO.md#18)
...
extend the role list to two corners; [#48](TODO.md#48) should reuse this same
...
## 73. ✅ No way to roll dice for something the system does not model — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
## 74. ✅ A crash can only be reached from a Chase Scene — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
## 75. ✅ Dermal armour negates flechette's damage-level increase; implant armour reaches the soak — *SR3 p.116* — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
([#48](TODO.md#48)). The result card says so instead. And the range band had to be threaded from the
...
[#86](TODO.md#86) as *"in no pack at all"* and needing to be created. They ship, correctly, under the
...
citation were the contacts' embedded stubs and that [#86](TODO.md#86)'s conversion would replace them,
so no separate work was needed. **[#86](TODO.md#86)'s conversion deliberately EXCLUDES cyberware** —
...
why it sits in [#86](TODO.md#86)'s "compound list" bucket and needs splitting before anything else.

**809 of 845 real pack cyberware items do carry a `bookPage`**, so the original point stands for
the packs — it is only the contacts' embedded stubs that lack citations, and they need [#86](TODO.md#86)
...
base + 2. Nothing protects them; a base rating corrected by [#84](TODO.md#84) silently moves them.
...
error from anything [#84](TODO.md#84) or [#88](#88) found: those were wrong VALUES on the right record.
This is the wrong record.

⚠ **Dock Worker was already one of the two hastily-entered records** — it shipped with no Karma
Pool in its note and with Willpower and Charisma transposed ([#83](TODO.md#83), [#84](TODO.md#84)). Three
...
⚠ **[#84](TODO.md#84) audited ATTRIBUTES only** — the stat-block row and the Dice Pools line. Skills,
...
⚠ **[#84](TODO.md#84)'s green result covered attributes ONLY**, and this is the cost of reading it more
...
rate on skills, assuming the rest is sound would be unwarranted. [#86](TODO.md#86)'s conversion will
...
left exactly where they are — those are [#86](TODO.md#86), a different extraction.
...
produces exactly the false confidence [#84](TODO.md#84) exists to avoid.
...
generator and had never been compared to a printed page. Given [#84](TODO.md#84) and [#89](#89) found
...
## 94. ✅ GM-initiated attack shows no GM difficulty modifiers — **DONE 2026-09-13** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
## 98. ✅ Dwarf resistance to disease and toxins — *SR3 p.56* — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
Not modelled. Split out of [#93](TODO.md#93)'s "known, not fixed here" list, 2026-09-12.
...
- **The consumer:** the attribute-roll dialog (`SR3EActorSheet._promptRollOptions`, reached from
  ...
  **unticked** checkbox — *"Resisting disease or toxin: +N (Dwarf resistance + Nephritic
  ...
  bonus channels*).
- A future drug/toxin flow ([#91](TODO.md#91) notes; the `drug` item type is reference-only) should
...
## 99. ✅ Remind the GM which vision the attacker has — natural or cyberware — **DONE 2026-09-12** (`fix/racial-mods`, awaiting the Foundry check in [#93](TODO.md#93))
...
⚠ **Cyberware detection is name-matching** — the same gap as [#18](TODO.md#18). The packs abbreviate
...
**2026-09-14 — gear: the field, where null means no rating.** Asked *"do we have reliable ratings for
...
turn a gear legacy `0` into null (219 contact gear items in the repo pack — **the install copy still
needs `node tools/patch-name-ratings.mjs --install` with Foundry closed**); `displayName` shows
